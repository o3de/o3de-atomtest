...
class TestHelper:
    ...
    def attach_component_to_entity(entityId, componentName):
        ...
        """
        typeIdsList = azlmbr.editor.EditorComponentAPIBus(azlmbr.bus.Broadcast, 'FindComponentTypeIdsByEntityType', [componentName], 0)
        ...
        componentOutcome = azlmbr.editor.EditorComponentAPIBus(azlmbr.bus.Broadcast, 'GetComponentOfType', entityId, typeIdsList[0])
        if componentOutcome.IsSuccess():
            return componentOutcome.GetValue() #In this case the value is not a list.
        componentOutcome = azlmbr.editor.EditorComponentAPIBus(azlmbr.bus.Broadcast, 'AddComponentsOfType', entityId, typeIdsList)
        if componentOutcome.IsSuccess():
    ...
    def get_property_list(Component):
        property_list = azlmbr.editor.EditorComponentAPIBus(azlmbr.bus.Broadcast, 'BuildComponentPropertyList', Component)
